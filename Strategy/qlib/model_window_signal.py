...
class ModelWindowSignal(Signal):
    """Lazily train a model and cache one score block per rebalance window."""
    ...
    def _build_window(self, window: ScheduledWindow) -> None:
        context = WindowContext(
            ...
            extra={"model_class": "LGBModel"},
        )

        selection_result = self.selector.select(self.factor_library, context)
        selected_items = selection_result.get("selected_items", []) or []
        training_result = self.trainer.train_and_predict(selected_items=selected_items, context=context)
        signal = training_result.get("signal")
        generation_context = training_result.get("generation_context")
        ...
        window_payload = {
            ...
            "selected_factor_ids": [item.get("factor", {}).get("factor_id") for item in selected_items],
            "selection_context": selection_result.get("selection_context"),
            "generation_context": generation_context,
            "model_class": "LGBModel",
            "signal_rows": 0 if signal is None else int(len(signal)),
        }
    ...
    def _ensure_named_index(signal: pd.DataFrame) -> pd.DataFrame:
        if isinstance(signal.index, pd.MultiIndex) and signal.index.nlevels == 2:
            ...
            if ModelWindowSignal._looks_like_datetime_level(level0):
                signal.index = signal.index.set_names(["datetime", "instrument"])
            elif ModelWindowSignal._looks_like_datetime_level(level1):
