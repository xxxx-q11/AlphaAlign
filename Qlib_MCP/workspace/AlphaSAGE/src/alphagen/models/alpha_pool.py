...
class AlphaPool(AlphaPoolBase):
    ...
    def to_dict(self) -> dict:
        ...
        # Convenient for downstream use: directly provides a mapping of {expression_string: ic}
        expr_ic_map = {expr: ic for expr, ic in zip(expr_strs, ics)}

        return {
            # List of expressions (backward compatible)
            "exprs": expr_strs,
            # New: expression-to-IC mapping for easy lookup by expression
            "expr_ic": expr_ic_map,
            "weights": list(self.weights[:self.size]),
